...
\begin{tikzpicture}[
    ...
    ptitle/.style={font=\bfseries\normalsize, text=kwdarkblue, anchor=north},
...
\node[ptitle] at ([xshift=0.18cm,yshift=-0.16cm]P1.north) {1. Inputs};
\node[ptitle] at ([xshift=0.18cm,yshift=-0.16cm]P2.north) {2. Assignment};
\node[ptitle] at ([xshift=0.18cm,yshift=-0.16cm]P3.north) {3. Classification view};
\node[ptitle] at ([xshift=0.18cm,yshift=-0.16cm]P4.north) {4. Confusion matrix};
\node[ptitle] at ([xshift=0.18cm,yshift=-0.16cm]P5.north) {5. Where are the \TN?};
...
% =====================  Panel 2: assignment  ===========================
\node[body, align=center, anchor=north] at ([yshift=-0.62cm]P2.north)
    {Match each prediction to a truth box};
\node[note, anchor=north] at ([yshift=-0.96cm]P2.north)
...
% Row 1: matched -> TP
\node[tbox] (T1) at ($(Q)+(1.15,-2.18)$) {$T_1$};
\node[pbox] (P1b) at ($(Q)+(3.05,-2.18)$) {$P_1$};
\draw[arrow, draw=kwgreen] (T1.east) -- (P1b.west);
\node at ($(Q)+(4.45,-2.18)$) {\TP};

% Row 2: truth with no prediction -> FN
\node[tbox] (T2) at ($(Q)+(1.15,-3.12)$) {$T_2$};
\node[eslot] (E1) at ($(Q)+(3.05,-3.12)$) {$\varnothing$};
\draw[darrow, draw=kworange] (T2.east) -- (E1.west);
\node at ($(Q)+(4.45,-3.12)$) {\FN};

% Row 3: prediction with no truth -> FP
\node[eslot] (E2) at ($(Q)+(1.15,-4.06)$) {$\varnothing$};
\node[pbox] (P2b) at ($(Q)+(3.05,-4.06)$) {$P_2$};
\draw[darrow, draw=kwred] (P2b.west) -- (E2.east);
\node at ($(Q)+(4.45,-4.06)$) {\FP};

\node[note, anchor=south] at ([yshift=0.14cm]P2.south)
    {solid = matched\enspace\textbullet\enspace dashed = no match ($\varnothing$)};
...
\draw[kwgreen, line width=1.2pt] ([xshift=0.048cm,yshift=-0.418cm]J2.north west) rectangle ([xshift=0.936cm,yshift=-2.290cm]J2.north west);
\draw[kwred,   line width=1.2pt] ([xshift=1.120cm,yshift=-0.154cm]J2.north west) rectangle ([xshift=1.472cm,yshift=-0.730cm]J2.north west);
\node[imtag, fill=kwgreen]                  at ([xshift=0.048cm,yshift=-0.418cm]J2.north west) {TP};
\node[imtag, fill=kwred, anchor=north east] at ([xshift=1.472cm,yshift=-0.154cm]J2.north west) {FP};
...
\coordinate (CMNW) at ([xshift=0.95cm,yshift=-1.18cm]P4.north west);
...
\node[note,align=center] at ($(CMNW)+(0.99,-0.84)$) {matched pair\\(here: 1)};
\node[body] at ($(CMNW)+(3.11,-0.40)$) {\FN};
\node[note,align=center] at ($(CMNW)+(3.11,-0.84)$) {missed truth\\(here: 1)};
\node[body] at ($(CMNW)+(0.99,-1.73)$) {\FP};
\node[note,align=center] at ($(CMNW)+(0.99,-2.17)$) {false box\\(here: 1)};
\node[body] at ($(CMNW)+(3.11,-1.73)$) {\TN};
\node[note,align=center] at ($(CMNW)+(3.11,-2.17)$) {empty region\\(here: undefined)};
...
\node[body, rotate=90] at ($(CMNW)+(-0.60,-1.28)$) {Real};
...
\node[body, align=left, text width=4.05cm, anchor=north west] at ([xshift=5.25cm,yshift=-0.90cm]P4.north west) {
%\textbf{View detection as classification.}\\[0.3em]
With the assignment fixed, we tally the true positives (\TP), false positives (\FP), and false negatives (\FN) --- one of each here, which supports many common metrics.\\[0.4em]
True negatives (\TN), though, are typically ignored in detection, which limits the metrics we can compute, like the MCC.
};
...
\foreach \i in {0,...,7}{
  \foreach \j in {0,...,11}{
    ...
    \ifnum\hit=0
      ...
      \draw[kwblue, line width=0.7pt, fill=kwblue, fill opacity=0.15]
...
\draw[kwblue, line width=1.0pt, fill=kwblue, fill opacity=0.08] ([xshift=0.10cm,yshift=-0.06cm]IT.north west) rectangle ([xshift=1.10cm,yshift=-0.39cm]IT.north west);
\draw[kwblue, line width=1.0pt, fill=kwblue, fill opacity=0.08] ([xshift=1.28cm,yshift=-0.15cm]IT.north west) rectangle ([xshift=1.94cm,yshift=-1.20cm]IT.north west);

% A scatter of varied boxes along the bottom strip of grass.
\draw[kwblue, line width=0.7pt, fill=kwblue, fill opacity=0.13] ([xshift=0.04cm,yshift=-2.70cm]IT.north west) rectangle ([xshift=0.36cm,yshift=-2.955cm]IT.north west);
\draw[kwblue, line width=0.9pt, fill=kwblue, fill opacity=0.10] ([xshift=0.26cm,yshift=-2.58cm]IT.north west) rectangle ([xshift=0.72cm,yshift=-2.91cm]IT.north west);
\draw[kwblue, line width=0.6pt, fill=kwblue, fill opacity=0.14] ([xshift=0.60cm,yshift=-2.76cm]IT.north west) rectangle ([xshift=0.88cm,yshift=-3.00cm]IT.north west);
\draw[kwblue, line width=1.0pt, fill=kwblue, fill opacity=0.08] ([xshift=0.80cm,yshift=-2.64cm]IT.north west) rectangle ([xshift=1.32cm,yshift=-2.97cm]IT.north west);
\draw[kwblue, line width=0.7pt, fill=kwblue, fill opacity=0.13] ([xshift=1.16cm,yshift=-2.79cm]IT.north west) rectangle ([xshift=1.48cm,yshift=-3.00cm]IT.north west);
\draw[kwblue, line width=0.9pt, fill=kwblue, fill opacity=0.10] ([xshift=1.40cm,yshift=-2.61cm]IT.north west) rectangle ([xshift=1.92cm,yshift=-2.955cm]IT.north west);

\node[imtag, fill=kwblue] at ([xshift=0.04cm,yshift=-0.04cm]IT.north west) {TN};

\node[body, align=left, text width=4.05cm, anchor=north west] at ([xshift=2.75cm,yshift=-0.85cm]P5.north west) {
What if we insist on counting \TN?
Every box you could draw that matches no real object is a \TN --- at every position and \emph{every} size.\\[0.35em]

The number is astronomically large. Rather than pick a value, we ask what happens if we take the limit and let \TN{} grow without bound: \TN${}\to\infty$.};
