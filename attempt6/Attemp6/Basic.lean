...
theorem tendsto_MCC_atTop_eq_FM
  ...
  have hDen := tendsto_den (TP := TP) (FP := FP) (FN := FN) hTPFPpos hTPFNpos
      ...
      MCC TP TN FP FN
          = (TP * TN - FP * FN) /
            Real.sqrt ((TP + FP) * (TP + FN) * (TN + FP) * (TN + FN)) := by
              ...
              simp [MCC, hAdef, mul_comm, mul_left_comm, mul_assoc]
      _   = (TP * TN - FP * FN) /
            Real.sqrt (A * (TN + FP) * (TN + FN)) := by
              -- just re-associate factors
              simp [hAdef, mul_comm, mul_left_comm, mul_assoc]
      _   = (TN * (TP - FP * FN / TN)) /
            Real.sqrt (A * (TN + FP) * (TN + FN)) := by
              simp [hNum]
      _   = (TP - FP * FN / TN) /
